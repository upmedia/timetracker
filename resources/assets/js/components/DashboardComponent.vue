<template>
  <div class="col-md-8 col-md-offset-2">
    <div class="no-projects" v-if="projects">
        ...
        <!-- <weekdays :selectedWeek="selectedWeek"></weekdays> -->
                    ...
                    <ul class="list-group" v-if="project.timers.length > 0">
                        <li v-for="timer in project.timers" :key="timer.id" class="list-group-item clearfix">
                            ...
                            <div class="pull-right">
                                ...
                                <span v-else>
                                  <duration :seconds="timer.seconds"></duration>
                                </span>
                                <button v-if="showTimerForProject(project, timer)"
...
<script>
  ...
  export default {
    ...
    created() {
      ...
      this.selectedWeek = this.currentWeek = moment().format('w')
    },
    ...
    methods: {
      ...
      _readableTimeFromSeconds: function(seconds) {
       ...
       startTimer: function(project, timer) {
        const start = moment(timer.started_at)

        this.counter.timer = timer
        this.counter.timer.project = project
        this.counter.seconds = parseInt(moment.duration(moment().diff(start)).asSeconds())
            ...
            .then(response => {
              ...
              this.projects.forEach(project => {
                if (project.id === parseInt(this.counter.timer.project.id)) {
                  // loop through the timers of the project and set stopped_at time
